    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            {/* Sort */}
            <div>
              <label htmlFor="sort-filter" className="sr-only">Sort tools by</label>
              <select
                id="sort-filter"
                value={sortBy}
            ...
            {/* View Mode Toggle */}
            <div className="flex border border-gray-300 dark:border-gray-700 rounded-md" role="group" aria-label="View mode">
              <button
                onClick={() => setViewMode('grid')}
                className={`p-2 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white ${
                  viewMode === 'grid'
      ...
      {filteredTools.length === 0 ? (
        <div className="text-center py-12">
          <Code2 className="h-12 w-12 text-gray-400 mx-auto mb-4" aria-hidden="true" />
            ...
            if (viewMode === 'grid') {
              return (
                <Link
                  key={tool.id}
                  href={tool.url}
                  className="group block p-6 border border-gray-200 dark:border-gray-800 rounded-lg hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-200 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
                  aria-label={`Open ${tool.name} - ${tool.shortDescription}`}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center">
                      <IconComponent className="h-6 w-6 text-black dark:text-white mr-3" aria-hidden="true" />
                      <div>
                    ...
                    <div className="flex items-center space-x-1">
                      {tool.featured && (
                        <Star className="h-4 w-4 text-yellow-500" fill="currentColor" aria-label="Featured tool" />
                      )}
                      {tool.popular && (
                        <Zap className="h-4 w-4 text-blue-500" aria-label="Popular tool" />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <ArrowRight className="h-4 w-4 text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors" aria-hidden="true" />
                  </div>
                </Link>
              );
            } else {
              return (
                <Link
                  key={tool.id}
                  href={tool.url}
                  className="group block p-4 border border-gray-200 dark:border-gray-800 rounded-lg hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
                  aria-label={`Open ${tool.name} - ${tool.shortDescription}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center flex-1 min-w-0">
                      <IconComponent className="h-5 w-5 text-black dark:text-white mr-3 flex-shrink-0" aria-hidden="true" />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {tool.featured && (
                            <Star className="h-3 w-3 text-yellow-500" fill="currentColor" aria-label="Featured" />
                          )}
                          {tool.popular && (
                            <Zap className="h-3 w-3 text-blue-500" aria-label="Popular" />
                          )}
                        </div>
      ...
      {/* Categories Overview */}
      <section className="mt-16 border-t border-gray-200 dark:border-gray-800 pt-12" aria-labelledby="categories-heading">
        <h2 id="categories-heading" className="text-2xl font-bold text-black dark:text-white mb-8 text-center">
          Browse by Category
        </h2>
          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className="p-6 border border-gray-200 dark:border-gray-800 rounded-lg hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-200 text-left group focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
                aria-label={`Filter tools by ${category.name} category`}
              >
